...
function SendModalViewModel() {
  ...
  self.dispNormalizedBalance = ko.computed(function() {
       return smartFormat(self.normalizedBalance(), null, 6);
  }, self);
  ...
  self.normalizedBalRemaining = ko.computed(function() {
    ...
    var balRemaining = Decimal.round(new Decimal(curBalance).sub(parseFloat(self.quantity())), 6, Decimal.MidpointRounding.ToEven).toFloat();
    if (self.asset() == 'ENRG')
      balRemaining = subFloat(balRemaining, normalizeQuantity(MIN_FEE))
    if(balRemaining < 0) return null;
    return balRemaining;
  }, self);

  self.dispNormalizedBalRemaining = ko.computed(function() {
       return smartFormat(self.normalizedBalRemaining(), null, 6);
  }, self);
...
function SweepModalViewModel() {
  ...
  self.selectedAssetsToSweep = ko.observableArray([]).extend({
    ...
    validation: {
      validator: function (val, self, callback) {
        ...
        self.missingBtcForFees = Math.max(REGULAR_DUST_SIZE, sweepingCost - self.btcBalanceForPrivateKey());
  ...
  self.addressForPrivateKey.subscribe(function(address) {
    ...
    self.sweepAssetsCost = {'ENRG': MIN_FEE+REGULAR_DUST_SIZE};
      ...
      failoverAPI("get_asset_info", {'assets': assets}, function(assetsData, endpoint) {
        ...
        WALLET.retriveBTCAddrsInfo([address], function(data) {
          ...
          if(unconfirmedRawBal > 0) {
            var viewModel = new SweepAssetInDropdownItemModel("ENRG", unconfirmedRawBal, normalizeQuantity(unconfirmedRawBal));
  ...
  self.addressForPrivateKeyForFees.subscribe(function(address) {
    ...
    WALLET.retriveBTCAddrsInfo([address], function(data) {
      $.jqlog.debug(data);
      self.addressForFeesBalanceMessage(normalizeQuantity(data[0]['confirmedRawBal'])+' ENRG in '+address);
      ...
      WALLET.BITCOIN_WALLET.getOldAddressesInfos(function(data) {
        for (var address in data) {
          if (self.excludedOldAddresses.indexOf(address) == -1) {
            self.availableOldAddresses.push(new BalancesAddressInDropdownItemModel(address, address, data[address]['ENRG']['privkey']));
          }       
        }
      }); 
    }    
  ...
  self.sendBtcForFees = function(callback) {
    ...
    for(var i = 0; i < self.selectedAssetsToSweep().length; i++) {
      var assetName = self.selectedAssetsToSweep()[i];
      if (assetName=='ENRG') sweepBTC = true;
    }
    if (sweepBTC) {
    ...
    var sendData = {
      ...
      encoding: 'auto',
      pubkey: pubkey,
      allow_unconfirmed_inputs: ALLOW_UNCONFIRMED
    };
  ...
  self.mergeOutputs = function(key, pubkey, callback, fees) {
    if (self.txoutsCountForPrivateKey>1) {
      ...
      var onTransactionError = function() {
        if (arguments.length==4) {
          var match = arguments[1].match(/Insufficient ENRG at address [^\s]+\. \(Need approximately ([\d]+\.[\d]+) ENRG/);
          if (match!=null) {
    ...
    multiAPIConsensus("create_issuance", transferData,
      function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
        ...
        WALLET.broadcastSignedTx(signedHex, function(issuanceTxHash, endpoint) { //broadcast was successful
          ...
          $.jqlog.debug("New ENRG balance: "+newBtcBalance);
  ...
  self._doSendAsset = function(asset, key, pubkey, opsComplete, adjustedBTCQuantity, callback) {
    ...
    if(asset == 'ENRG') assert(adjustedBTCQuantity !== null);
    ...
    var quantity = (asset == 'ENRG') ? (self.btcBalanceForPrivateKey() - MIN_FEE) : selectedAsset.RAW_BALANCE;
    var normalizedQuantity = (asset == 'ENRG') ? normalizeQuantity(quantity) : selectedAsset.NORMALIZED_BALANCE;
    
    assert(selectedAsset);
    
    if(!quantity) { //if there is no quantity to send for the asset, only do the transfer
      if(asset == 'XEP' || asset == 'ENRG') { //nothing to send, and no transfer to do
    ...
    multiAPIConsensus("create_send", sendData,
      function(unsignedTxHex, numTotalEndpoints, numConsensusEndpoints) {
  ...
  self.doAction = function() {
    ...
    if(hasBTC !== false) {
      ...
      sendsToMake.push(["ENRG", cwk, pubkey, opsComplete, self.btcBalanceForPrivateKey()]);
    }
    ...
    var launchSweep = function() {
      if (sendsToMake.length==1 && sendsToMake[0][0]=='ENRG') {
        doSweep();
      } else {
...
function BroadcastModalViewModel() {
  ...
  self.doAction = function() {
    var params = {
      source: self.address(),
      fee_fraction: Decimal.round(new Decimal(self.feeFraction()).div(100), 6, Decimal.MidpointRounding.ToEven).toFloat(),
      text: self.textValue(),
