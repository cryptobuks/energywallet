...
function AssetViewModel(props) {
  ...
  self.rawBalance = ko.observable(props['rawBalance'] || (self.ASSET == 'ENRG' ? null : 0));
  ...
  self.dispEscrowedBalance = ko.computed(function() {
    if (parseFloat(self.escrowedBalance()) > 0) {
      return '/ OnHold: ' + smartFormat(normalizeQuantity(self.escrowedBalance(), self.DIVISIBLE));
    }
  }, self);
  ...
  self.isMine = ko.computed(function() {
    if(self.ASSET == 'ENRG' || self.ASSET == 'XEP') return null;
  ...
  self.dispBalance = ko.computed(function() {
    ...
    var unconfirmed = self.availableBalance() != self.normalizedBalance() ? ' <span style="font-size:11px">(' + smartFormat(self.unconfirmedBalance(), true) + ')</span>' : '';
