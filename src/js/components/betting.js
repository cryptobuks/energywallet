  ...
  $.jqlog.debug("displayFeed1"); 
    ...
    for(var i = 0; i < addresses.length; i++) {
      options.push({
        ...
        label: addresses[i][1] + ' (' + addresses[i][2] + ' XEP)'
      });
    ...
    for (var i in feed.counters.bets) {
       feed.counters.bets[i].wager_quantity = normalizeQuantity(feed.counters.bets[i].wager_quantity) + ' XEP';
       feed.counters.bets[i].wager_remaining = normalizeQuantity(feed.counters.bets[i].wager_remaining) + ' XEP';
