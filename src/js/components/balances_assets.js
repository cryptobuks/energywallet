...
ko.validation.rules['assetNameIsTaken'] = {
  ...
  validator: function (val, self, callback) {
    if (hasLowerCase(val)) return false;
    else if (val == 'ENRG') return false;
...
function CreateAssetModalViewModel() {
  ...
  self.name = ko.observable('').extend({
    ...
    validation: {
      validator: function (val, self) {
        if ((!val) || (hasLowerCase(val)))  return false;
        else if (((byteCount(val) < 4) && (val != 'XEP')) || (val == 'ENRG'))  return false;
        return true;
      },
      params: self
    }
...
function PayDividendModalViewModel() {
  ...
  self.assetName = ko.observable('').extend({
    ...
    validation:  {
      validator: function (val, self) {
        if(!self.assetData()) return false; //wait until dividend asset chosen to validate
        if ((!val) || (hasLowerCase(val)))  return false;
        else if (((byteCount(val) < 4) && (val != 'XEP')) || (val == 'ENRG'))  return false;
  ...
  self.assetName.subscribe(function(name) {
    if ((!name) || (hasLowerCase(name))) return;
    else if (((byteCount(name) < 4) && (name != 'XEP')) || (name == 'ENRG')) return;
  ...
  self.totalPay = ko.computed(function() {
    ...
    return Decimal.round(totalPay, 6, Decimal.MidpointRounding.ToEven).toFloat();
  ...
  self.dividendAssetBalRemainingPostPay = ko.computed(function() {
    ...
    return Decimal.round(new Decimal(self.dividendAssetBalance()).sub(self.totalPay()), 6, Decimal.MidpointRounding.ToEven).toFloat();
  }, self);
  ...
  self.doAction = function() {
    ...
    if (self.selectedDividendAsset() == 'ENRG') {
      var params = {
        'filters': [
  ...
  self.showModal = function(address, resetForm) {
    ...
    failoverAPI("get_normalized_balances", {'addresses': [address.ADDRESS]}, function(data, endpoint) {
      ...
      WALLET.retrieveBTCBalance(address.ADDRESS, function(balance) {
        if(balance) {
          self.availableDividendAssets.unshift(new DividendAssetInDropdownItemModel("ENRG", balance, normalizeQuantity(balance)));
        }
      });
    });
  }
