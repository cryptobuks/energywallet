...
function BalanceHistoryViewModel() {
  ...
  self.availableAssets = ko.observableArray(["XEP"]);
  self.graphData = null;
  ...
  self.assetChanged = function() {
    ...
    if(self.selectedAsset() == "ENRG") {
      var addresses = WALLET.getAddressesList();
      self.graphData = [];
      
      for(var i=0; i < addresses.length; i++) {
        var q = 'select * from html where url="http://energychain.info/charts/balance?format=json&address='+addresses[i]+'"';
        $.queryYQL(q, function(data, textStatus, jqXHR) {
          ...
          var address = /address%3D([A-Za-z0-9]+)%22/g.exec(jqXHR.url)[1];
          var addressHash = hashToB64(address);
          $.jqlog.debug( "Could not get ENRG balance from blockchain for address " + address + ": " + errorThrown);
          var addressName = PREFERENCES['address_aliases'][addressHash] ? "<b>" + PREFERENCES['address_aliases'][addressHash] + "</b> (" + address + ")" : address; 
...
function TransactionHistoryItemViewModel(data) {
  ...
  self.dispDescription = function() {
    ...
    if(self.RAW_TX_TYPE == 'burns') {
      ...
      desc = i18n.t("hist_send", smartFormat(normalizeQuantity(self.DATA['quantity'], self.DATA['_divisible'])), self.DATA['asset'], getAddressLabel(self.DATA['destination']));
      ...
      if(self.DATA['forward_asset'] == 'ENRG' || self.DATA['backward_asset'] == 'ENRG') {
        desc += " <b>(" + i18n.t("pending ENRGpay") + ")</b>";
      }
      ...
      if(self.DATA['transfer']) {
        desc = i18n.t("hist_transfer", self.DATA['asset'], getAddressLabel(self.DATA['issuer']));
      ...
      desc = i18n.t("hist_bet", BET_TYPES[self.DATA['bet_type']],
        getAddressLabel(self.DATA['feed_address']),
      ...
      desc = i18n.t("hist_bet_match", getAddressLabel(self.DATA['feed_address']),
      ...
      desc = i18n.t("hist_credit_debit", getAddressLabel(self.DATA['address']), tx_type,
