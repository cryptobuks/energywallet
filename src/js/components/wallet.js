...
function WalletViewModel() {
  //The user's wallet
  var self = this;
  self.BITCOIN_WALLET = null;
  self.autoRefreshBTCBalances = true;
  ...
  self.updateBalance = function(address, asset, rawBalance, unconfirmedRawBal) {
    ...
    var assetObj = addressObj.getAssetObj(asset);
    if(!assetObj) {
      assert(asset != "XEP" && asset != "ENRG", "ENRG or XEP not present in the address?");
      ...
      if (asset == 'ENRG' && unconfirmedRawBal) {
      ...
      function(balancesData, endpoint) {
        ...
        if(!balancesData.length) {
          for (var i in addresses) {
            WALLET.getAddressObj(addresses[i]).addOrUpdateAsset('XEP', {}, 0, 0);
          }
        ...
        failoverAPI("get_escrowed_balances", {'addresses': addresses}, function(escrowedBalances) {
          ...
          failoverAPI("get_asset_info", {'assets': assets}, function(assetsInfo, endpoint) {
            ...
            for (i=0; i < assetsInfo.length; i++) {
                if (assetsInfo[i]['asset'] == 'ENRG') continue;
  ...
  self.refreshBTCBalances = function(isRecurring, addresses, onSuccess) {
    ...
    var pendingActionsHasBTCSend = ko.utils.arrayFirst(PENDING_ACTION_FEED.entries(), function(item) {
      return item.CATEGORY == 'sends' && item.DATA['asset'] == 'ENRG';
    });
    ...
    self.retriveBTCAddrsInfo(addresses, function(data) {
      ...
      for(var i=0; i < data.length; i++) {
        self.updateBalance(data[i]['addr'], "ENRG", data[i]['confirmedRawBal'], data[i]['unconfirmedRawBal']);
        
        addressObj = self.getAddressObj(data[i]['addr']);
        assert(addressObj, "Cannot find address in wallet for refreshing ENRG balances!");
      ...
      var addressObj = null;
      for(var i=0; i < addresses.length; i++) {
        self.updateBalance(addresses[i], "ENRG", null, null); //null = UNKNOWN
        addressObj = self.getAddressObj(addresses[i]);
  ...
  self.retrieveBTCBalance = function(address, onSuccess, onError) {
    failoverAPI("get_chain_address_info", {"addresses": [address], "with_uxtos": false, "with_last_txn_hashes": 0},
      function(data, endpoint) {
        return onSuccess(
          parseInt(data[0]['info']['balanceSat'] || 0),
          parseInt(data[0]['info']['unconfirmedBalanceSat'] || 0)
        );
      },
  ...
  self.canDoTransaction = function(address) {
    ...
    if(self.getBalance(address, "ENRG", false) < MIN_PRIME_BALANCE) {
  ...
  self.doTransaction = function(address, action, data, onSuccess, onError) {
    ...
    if (ALLOW_UNCONFIRMED && supportUnconfirmedChangeParam(action)) {
      data['allow_unconfirmed_inputs'] = false;
    }
    ...
    } else if (action == "create_dividend" && data['dividend_asset'] == 'ENRG') {
